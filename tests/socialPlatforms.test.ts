...
/**
 * Audit 8.26 — native per-platform download commands: thin wrappers over the
 * hardened `.download` pipeline, independent of the legacy corpus (removed
 * 8.56). 8.57: tiktok + instagram are the ACTIVE commands (owner request);
 * facebook/youtube stay as matcher entries only (`.dl`/`.ytv` cover them),
 * so their command files are intentionally NOT restored.
 */
