...
| 🧩 | **Native command set** (`.a` anime, `.m` media toolkit, `.dl` multi-platform downloads, `.w` episode watch, `.ai`/`.image`, group moderation…) + sandboxed panel-created ones (no fs/process/network, no restart) — the 145-file legacy corpus and its CJS bridge were REMOVED (audit 8.56) |
...
- **Vocal separation (IA)** — reply to a song with `.m karaoké` (instrumental) or `.m voix` (acapella): independent MDX-Net ONNX sidecar (venv + 66.8 MB model installed by `nebula setup`, sha256-pinned), chunked CPU inference under a RAM cap, one track at a time (≤ 8 min)
